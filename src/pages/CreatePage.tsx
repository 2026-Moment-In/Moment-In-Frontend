...
export default function CreatePage() {
  ...
  // ❌ 기존 handlePublish 함수를 찾아서 아래 코드로 완전히 교체합니다!
const handlePublish = async () => {
  // 1. 중복 클릭 방지 및 로딩 켜기
  setSubmitting(true);

  try {
    // 2. 백엔드 규격에 맞게 전송 데이터(Body) 조립하기
    const requestData = {
      groomName: groomName,
      brideName: brideName,
      ...
      // 필요한 추가 데이터나 에디터 데이터가 있다면 백엔드와 상의 후 여기에 추가
    };

    // 에디터 타입 딴지 방지를 위해 'as any' 처리
    const result = await api.createMyWedding(requestData as any);

    console.log("백엔드 저장 성공 결과:", result);

    // 4. 백엔드가 리턴해 준 실제 접속 코드(code) 추출
    // 백엔드 응답 형태: { code: "abc12345", wedding: {...} }
    const generatedCode = result.code;

    if (!generatedCode) {
      throw new Error("백엔드에서 접속 코드가 정상적으로 오지 않았습니다.");
    }

    showToast("🎉 모먼트인 청첩장이 성공적으로 발행되었습니다!");
    
    setTimeout(() => {
      navigate(`/qr/${generatedCode}`);
    }, 1000);
